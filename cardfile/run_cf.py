...
argParser.add_argument("--year",           action='store',      type=int, default=2016, choices = [ 2016, 2017, 20167, 201678 ], help='Which year?')
...
argParser.add_argument("--sample",         action='store',      type=str, choices = [ 'ttz','tttt' , 'twz' ], help="?")
...
###lumi lumi_year = {2016: 35900.0, 2017: 41500.0, 2018: 59970.0}
...
if args.sample == 'ttz' : 
    plotfilename.append( '/mnt/hephy/cms/rosmarie.schoefbeck/www/tWZ/plots/analysisPlots/TTZ_DY_noData/Run2016/all/dilepM-onZ1-minDLmass12-njet5p-btag1p/ttz_dy_TTZ.root' )
    plotfilename.append( '/mnt/hephy/cms/rosmarie.schoefbeck/www/tWZ/plots/analysisPlots/TTZ_DY_noData/Run2016/all/dilepM-onZ1-minDLmass12-njet5p-btag1p/ttz_dy_LSTM_TTZ.root' )
#elif args.sample == 'twz': 
else: 
    plotfilename.append( '/mnt/hephy/cms/rosmarie.schoefbeck/www/tWZ/plots/analysisPlots/tttt_3l_v3_noData/RunII/all/trilep4tM-offZ1-minDLmass12-njet4p-btag2p/tttt_ttw_ttz_nonprompt_TTTTcoarse.root' )
    plotfilename.append( '/mnt/hephy/cms/rosmarie.schoefbeck/www/tWZ/plots/analysisPlots/tttt_3l_v3_noData/RunII/all/trilep4tM-offZ1-minDLmass12-njet4p-btag2p/tttt_ttw_ttz_nonprompt_LSTM_TTTTcoarse.root' )
...
for filename in plotfilename: 
    ...
    if args.sample == 'ttz' :
        ...
        estimates['signal']= sig 
        estimates['TTW']=canvas.GetListOfPrimitives().At(1)
        estimates['TTZ']=canvas.GetListOfPrimitives().At(2)
        estimates['nonpromt']=canvas.GetListOfPrimitives().At(4)
        estimates['WZ']=canvas.GetListOfPrimitives().At(5)
    #print estimates 
...
#            for e in estimates: print e 
            ...
            for e in estimates:  
                ...
                if args.sample == 'tttt' :
                    ...
                    if e == 'signal':
                        expected = expected - estimates['nonpromt'].GetBinContent(b+1)
                    if e == 'TTW':
                        expected = expected - estimates['TTZ'].GetBinContent(b+1)
                    if e == 'nonpromt':
                        expected = expected - estimates['WZ'].GetBinContent(b+1)
                    if e == 'TTZ':
                        expected = expected - estimates['signal'].GetBinContent(b+1)
                ...
                if args.lumi: 
                    ...
                    if args.sample == 'ttz' :
                        ...
                        expected = (expected/137)*args.lumi 
...
#                        expected = expected - (estimates['DY'].GetBinContent(b+1)/35.900)*args.lumi
                ...
                print e,expected, b,estimates[e].GetBinContent(b+1)
...
#            logger.info('Adding signal %s'%(sigval * xSecScale ))
    ...
    limitmed = limit['0.500'] 
    print limit
    print limitmed
    txtfilename = args.sample + ".txt"
    outfile = open(txtfilename, 'a')
    if not "LSTM" in filename : outfile.write(str(args.lumi)+' '+str(limitmed))
    else  : outfile.write(' '+str(limitmed)+'\n')
    outfile.close()
