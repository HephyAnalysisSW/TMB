...
for i_w, w in enumerate(ws):
    w.set_order(2)
    w.get_fisher_weight_string(args.WC, args.WC)

fisher_plots  = [ samples[i_w].get1DHistoFromDraw(
                  "TMath::Log10("+w.get_fisher_weight_string(args.WC, args.WC)+")", 
                  [270, -25,2], 
                  selectionString = "(1)", 
...
p = Plot.fromHisto(name = "fisher", histos = [[h] for h in fisher_plots + fisher_plots_cumulative], texX = "log_{10}(I_{F})" , texY = "" )
plotting.draw(p, plot_directory = os.path.join(plot_directory, args.plot_directory), ratio = None, logY = True, logX = False, yRange = (10**-3, "auto")) 
