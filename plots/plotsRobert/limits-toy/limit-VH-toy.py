...
#        eft     = { k:eft[k]/norm for k in model.wilson_coefficients }
#    else:
...
q_event = make_q( test_statistic, truth=truth, **{WC:theta} )

toys_sm = make_toys( lumi_factor*lambda_tot(), n_toys )
toys_theta = make_toys( lumi_factor*lambda_tot(**{WC:theta}), n_toys, **{WC:theta})

q_theta_given_theta = np.array([np.sum( q_event[toy_] ) for toy_ in toys_theta ])
q_theta_given_SM    = np.array([np.sum( q_event[toy_] ) for toy_ in toys_sm ])
...
#                for i_theta, theta in enumerate( theta_vals ):
#
#                    q_event = make_q( test_statistic, truth=truth, **{WC:1} )
#
#                    #log_sigma_tot_ratio_subtraction = np.log(sigma_tot_ratio(theta)) if not extended else 0
