...
from TMB.Tools.objectSelection           import lepString 
# Analysis
...
argParser.add_argument('--plot_directory', action='store', default='TMB')
...
read_variables.extend( mva_config.mva_read_variables )
...
for i_mode, mode in enumerate(allModes):
    ...
    plots.append(Plot(
      ...
      binning=[len(allModes), 0, len(allModes)],
    ))
...
for mode in ["all"]:
    ...
    if mode == "all": drawPlots(allPlots['mu'], mode, dataMCScale)
