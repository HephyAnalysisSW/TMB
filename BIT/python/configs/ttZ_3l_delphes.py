...
weight_variables = ['cpQM', 'cpQ3', 'cpt', 'ctZ', 'ctZI', 'ctG', 'ctGI']
max_order        = 2

from TMB.Samples.delphes_benchmarks import *
training_samples = [ ttZ_ll_LO_order2_15weights_ref_ext_delphes_RunII ]

assert len(training_samples)==len(set([s.name for s in training_samples])), "training_samples names are not unique!"

reweight_pkl     = ttZ_ll_LO_order2_15weights_ref_ext_delphes_RunII.reweight_pkl
...
selectionString = cutInterpreter.cutString( 'trilep-onZ-njet3p' )
...
def load(directory = '/mnt/hephy/cms/$USER/BIT/models/default/ttZ_3l_delphes/', bit_derivatives=bit_derivatives):
    import sys, os
